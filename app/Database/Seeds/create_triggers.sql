...
BEFORE INSERT ON Commande FOR EACH ROW BEGIN
	IF NEW.id_coupon IS NOT NULL AND (SELECT MAX(c.utilisations_max) FROM Coupon c WHERE c.id_coupon = NEW.id_coupon) <= (SELECT COUNT(*) FROM Commande c WHERE c.id_coupon = NEW.id_coupon)
	THEN
