...
CREATE TRIGGER coupon_trop_utilise_update
...
CREATE TRIGGER coupon_expire_insert
BEFORE INSERT ON Commande FOR EACH ROW BEGIN
...
CREATE TRIGGER coupon_expire_update BEFORE
...
CREATE TRIGGER coupon_non_valable_insert BEFORE
...
CREATE TRIGGER coupon_non_valable_update BEFORE
UPDATE ON Commande FOR EACH ROW BEGIN
IF NEW.id_coupon IN (SELECT id_coupon FROM Coupon WHERE est_valable = false) THEN
    SIGNAL SQLSTATE '45000' SET message_text = 'Ce coupon de réduction n\'est pas valable.';
END IF;
...
CREATE TRIGGER supprimer_client_cascade BEFORE
...
CREATE TRIGGER exemplaire_pas_dispo_commande BEFORE
...
CREATE TRIGGER categorie_produit_invalide_update BEFORE
...
CREATE TRIGGER taille_valide_insert BEFORE
...
CREATE TRIGGER taille_valide_update BEFORE
INSERT ON Exemplaire FOR EACH ROW BEGIN
   IF NEW.taille IS NOT NULL THEN
       IF NEW.id_produit IN (SELECT categorie FROM Produit WHERE categorie = 'accessoire') THEN
           SIGNAL SQLSTATE '45000' SET message_text = 'Les accessoires n\'ont pas de taille.';
...
CREATE TRIGGER update_commande_validee BEFORE
UPDATE ON Commande FOR EACH ROW BEGIN
...
CREATE TRIGGER commande_validee_mauvaise BEFORE
UPDATE ON Commande FOR EACH ROW BEGIN
